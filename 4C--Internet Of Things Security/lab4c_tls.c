...
void shut_down()
{
    ...
    char buffer [20];
    size = sprintf(buffer, "%02d:%02d:%02d SHUTDOWN\n", l_time->tm_hour, l_time->tm_min, l_time->tm_sec);
...
int main(int argc, char *argv[])
{
    ...
    stat = SSL_set_fd(ssl, socketFildes);
    if (stat <= 0)
        Error("associating the socket file descriptor with SSL");
